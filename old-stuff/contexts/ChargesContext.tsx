...
// Helpers
import { formatPriceToString } from '@/old-stuff/old-lib/helpers'

// Types
import { InvoiceType, ItemType } from '@/old-stuff/old-lib/types'
...
export const ChargesContextProvider = ({ children }: ChargesContextProps) => {
  ...
  const [discountSwitch, setDiscountSwitch] = useState<boolean>(discount?.amount ? true : false)
  const [taxSwitch, setTaxSwitch] = useState<boolean>(tax?.amount ? true : false)
  const [shippingSwitch, setShippingSwitch] = useState<boolean>(shipping?.cost ? true : false)
  ...
  useEffect(() => {
    calculateTotal()
  }, [itemsArray, totalInWordsSwitch, discountType, discount?.amount, taxType, tax?.amount, shippingType, shipping?.cost, currency])
  ...
  const calculateTotal = () => {
    ...
    const totalSum: number = itemsArray.reduce((sum: number, item: ItemType) => sum + Number(item.total), 0)
