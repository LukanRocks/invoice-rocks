...
// Currencies
import currenciesDetails from '@/public/data/currencies.json'
import { CurrencyDetails } from '@/old-stuff/old-lib/types'
...
const formatPriceToString = (price: number, currency: string): string => {
  ...
  const integerPartInWords = numberToWords.toWords(integerPart).replace(/^\w/, (c) => c.toUpperCase())
 ...
 */
const flattenObject = <T>(obj: Record<string, T>, parentKey = ''): Record<string, T> => {
...
export { formatNumberWithCommas, formatPriceToString, flattenObject, isValidEmail, isDataUrl, getInvoiceTemplate, fileToBuffer }
