...
// Components
import { DrawSignature, TypeSignature, UploadSignature } from '@/app/components'
...
const SignatureModal = ({}: SignatureModalProps) => {
  ...
  const { handleCanvasEnd, signatureData, typedSignature, selectedFont, uploadSignatureImg, signatureRef } = useSignatureContext()
    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger className='flex justify-start'>
          <div>
            ...
            {signature && isDataUrl(signature) ? (
              <img className='border border-black rounded-md hover:border-blue-500 bg-white' src={signature} width={300} alt='' />
        ...
        <DialogContent className='select-none'>
          <DialogTitle>{_t('form.steps.summary.signature.heading')}</DialogTitle>

          <Tabs value={tab} onValueChange={onTabChange}>
            <TabsList className='grid w-full grid-cols-3'>
              <TabsTrigger value={SignatureTabs.DRAW}>{_t('form.steps.summary.signature.draw')}</TabsTrigger>
              <TabsTrigger value={SignatureTabs.TYPE}>{_t('form.steps.summary.signature.type')}</TabsTrigger>
              <TabsTrigger value={SignatureTabs.UPLOAD}>{_t('form.steps.summary.signature.upload')}</TabsTrigger>
            </TabsList>
