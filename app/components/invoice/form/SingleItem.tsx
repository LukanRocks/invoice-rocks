...
const SingleItem = ({ name, index, fields, field, moveFieldUp, moveFieldDown, removeField }: SingleItemProps) => {
  ...
  const boxDragClasses = isDragging ? 'border-2 bg-gray-200 border-blue-600 dark:bg-slate-900 z-10' : 'border'

  const gripDragClasses = isDragging ? 'opacity-0 group-hover:opacity-100 transition-opacity cursor-grabbing' : 'cursor-grab'
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-wrap justify-between gap-y-5 gap-x-2' key={index}>
        <FormInput name={`${name}[${index}].name`} label={_t('form.steps.lineItems.name')} placeholder='Item name' vertical />
        ...
        <div className='flex flex-col gap-2'>
          ...
          <Input value={`${total} ${currency}`} readOnly placeholder='Item total' className='border-none font-medium text-lg bg-transparent' size={10} />
        </div>
      </div>
      <FormTextarea name={`${name}[${index}].description`} label={_t('form.steps.lineItems.description')} placeholder='Item description' />
      <div>
