...
const DrawSignature = ({ handleSaveSignature }: DrawSignatureProps) => {
  const { signatureData, signatureRef, colors, selectedColor, handleColorButtonClick, clearSignature, handleCanvasEnd } = useSignatureContext()
    ...
    <TabsContent value={SignatureTabs.DRAW}>
      <Card className='border-none shadow-none'>
        ...
        <div className='flex justify-between gap-2 pt-2'>
          {/* Color selector */}
          <SignatureColorSelector colors={colors} selectedColor={selectedColor} handleColorButtonClick={handleColorButtonClick} />

          {signatureData && (
            <BaseButton tooltipLabel='Clear the signature board' variant='outline' className='w-fit gap-2' onClick={clearSignature}>
              Erase
              <Eraser />
            </BaseButton>
          )}
          <BaseButton tooltipLabel='Save changes' disabled={!signatureData} onClick={handleSaveSignature}>
            Done
            <Check />
          </BaseButton>
        </div>
