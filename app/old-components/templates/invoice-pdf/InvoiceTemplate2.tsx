  ...
  return (
    <InvoiceLayout data={data}>
        ...
        <div className='sm:text-right space-y-2'>
          <div className='grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2'>
            <dl className='grid sm:grid-cols-6 gap-x-3'>
              ...
              <dd className='col-span-3 text-gray-500'>{new Date(details.invoiceDate).toLocaleDateString('en-US', DATE_OPTIONS)}</dd>
            </dl>
            <dl className='grid sm:grid-cols-6 gap-x-3'>
              <dt className='col-span-3 font-semibold text-gray-800'>Due date:</dt>
              <dd className='col-span-3 text-gray-500'>{new Date(details.dueDate).toLocaleDateString('en-US', DATE_OPTIONS)}</dd>
            </dl>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <p className='text-gray-500 text-sm'>If you have any questions concerning this invoice, use the following contact information:</p>
        <div>
      ...
      {details?.signature?.data && isDataUrl(details?.signature?.data) ? (
        <div className='mt-6'>
          ...
          <img src={details.signature.data} width={120} height={60} alt={`Signature of ${sender.name}`} />
        </div>
